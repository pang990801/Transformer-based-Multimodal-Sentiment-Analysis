...
if __name__ == '__main__':
    filepath = 'data/MOSEI/mosei_raw_glove.pkl'
    batch_size = 32
    max_seq_len = 50
    ...
    for modalities in modality_combinations:
        ...
        # 加载完整数据
        traindata, validdata, testdata = get_ablation_dataloader(filepath, embedding='glove',batch_size=32, data_type='mosei' ,max_pad=True,num_workers=0, robust_test=False, modalities=modalities)
        ...
        #       lr=1e-4, save=f"checkpoints/ablation/model_glove_{'+'.join(modalities)}.pt", weight_decay=0.01, objective=torch.nn.L1Loss())

        # 测试
        print("Testing...")
        model = torch.load(f"checkpoints/ablation/model_glove_{'+'.join(modalities)}.pt").cuda()
        test_results = test(model=model, test_dataloaders_all=testdata, dataset='mosei',
                            is_packed=False, criterion=torch.nn.L1Loss(), no_robust=True)
    ...
    with open('ablation_glove_results.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Fusion Method', 'MAE', 'ACC7', 'Acc5', 'ACC2', 'Corr', 'F1'])
        writer.writerows(results)

    print("All experiments completed. Results saved to ablation_glove_results.csv.")